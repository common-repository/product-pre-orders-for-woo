...
class WPRO_WOO_PRE_ORDER_Frontend_single_product {
	public function __construct() {
		...
		if ( $get_option['enabled'] == 'yes' ) {
			...
			//add to cart text
			add_filter( 'woocommerce_product_single_add_to_cart_text', array( $this, 'pre_order_label_simple' ), 20, 2 );
			add_filter( 'woocommerce_product_add_to_cart_text', array( $this, 'pre_order_label_simple' ), 10, 2 );
			//text cart
			...
			//Change Price simple
			add_filter( 'woocommerce_product_get_price', array( $this, 'pre_order_price_change_product_simple' ), 10, 2 );
			add_filter( 'woocommerce_product_get_sale_price', array( $this, 'pre_order_price_change_product_simple' ), 10, 2 );
			// Change Price variable
			add_action( 'woocommerce_product_variation_get_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			add_filter( 'woocommerce_product_variation_get_sale_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			add_filter( 'woocommerce_variation_prices_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			add_filter( 'woocommerce_variation_prices_sale_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			add_filter( 'woocommerce_variation_prices_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			add_filter( 'woocommerce_variation_prices_sale_price', array( $this, 'pre_order_price_variable' ), 10, 2 );
			//display markup
			add_filter( 'woocommerce_get_price_html', array( $this, 'pre_order_display_price_markup_discount_variation' ), 10, 2 );
			add_filter( 'woocommerce_variable_get_price_html', array( $this, 'pre_order_display_price_markup_discount_variation' ), 10, 2 );
	...
	public function script_and_css_frontend() {
		wp_enqueue_script( 'product-pre-orders-for-woo-setting-frontend-js', WPRO_WOO_PRE_ORDER_JS . 'product-pre-orders-for-woo-setting-frontend.js', array( 'jquery' ), WPRO_WOO_PRE_ORDER_VERSION, true );
		wp_enqueue_style( 'product-pre-orders-for-woo-setting-frontend', WPRO_WOO_PRE_ORDER_CSS . 'product-pre-orders-for-woo-setting-frontend.css', array(), WPRO_WOO_PRE_ORDER_VERSION );
		wp_register_style( 'product-pre-orders-for-woo-style-css', false, array(), WPRO_WOO_PRE_ORDER_VERSION );
		...
		if ( $cart_item['data']->is_type( 'simple' ) ) {
			...
			if ( ! empty( $pre_date ) ) {
                ...
                <div class="wpro-pre-order-availability-date-cart">
					<?php
					if ( $no_date_label ) {
						echo esc_html( $no_date_label );
					} else {
						echo esc_html( $get_option['no_date_text'] );
					}
					?>
                </div>
				<?php
