...
class WPRO_WOO_PRE_ORDER_Admin_Product {
    ...
    public function __construct() {
        ...
        if ( $get_option['enabled'] == 'yes' ) {
            ...
            add_action( 'admin_print_styles', array( $this, 'pre_order_panel_style'));
            add_filter( 'woocommerce_product_data_tabs', array( $this, 'add_my_custom_product_data_tab' ), 10, 1 );
            add_action( 'woocommerce_product_data_panels', array( $this, 'wpro_pre_order_data_custom_field_simple' ) );
            add_action( 'woocommerce_process_product_meta', array( $this, 'pre_order_save_simple_data' ) );
            add_action( 'woocommerce_product_after_variable_attributes', array( $this, 'wpro_pre_order_data_custom_field_variation' ), 10, 3 );
    ...
    public function pre_order_panel_style() {
        ?>
        <style>
            #woocommerce-product-data ul.wc-tabs li a[href="#wpro_pre_order"]::before {
                content: "\f174";
            }
            .form-row input._wpro_date_variable[type=text] {
                width: 364px;
            }
            ._wpro_dates_field_variable .form-row input._wpro_time_variable[type=time] {
                width: 145px;
            }
            #pre_order_date {
                width: 12%;
     ...
     */
    public function wpro_pre_order_data_custom_field_simple() {
        global $post;
        echo '<div class="panel woocommerce_options_panel" id = "wpro_pre_order" >';
        echo wp_kses_post( wp_nonce_field( 'wpro_product_meta_box', '_wpro_nonce' ) );
        ...
        echo '<p class="form-field _wpro_dates_field">
					...
					</span>' . wp_kses_post( wc_help_tip( esc_html__( 'Set the date when the product will be for sale. The Timezone used is the WorldPress local Timezone. Settings -> General -> Timezone.', 'product-pre-orders-for-woo' ) ) ) . '</p>';
        ...
        <div class="wpro-form-manage-price hidden">
            ...
            echo '<p class="form-field _wpro_price">
					...
					</span>' . wp_kses_post( wc_help_tip( esc_html__( 'Set the quantity to apply (Fixed or percentage). Type numbers only.', 'product-pre-orders-for-woo' ) ) ) . '</p>';
            ?>
        </div>
        <?php
        echo '</div>';
    ...
    public function add_pre_order_variable_checkbox( $loop, $variation_data, $variation ) {
        ...
        <label>
            ...
            <?php echo esc_html( _x( 'Pre-Order', 'product-pre-orders-for-woo' ) ); ?>
            <?php echo wp_kses_post( wc_help_tip( esc_html__( 'Enable this option to set this variation to the Pre-Order status.', 'product-pre-orders-for-woo' ) ) ); ?>
        </label>
        <?php
    }
     ...
     * @param $variation
     */
    public function wpro_pre_order_data_custom_field_variation( $loop, $variation_data, $variation ) {
        ...
        echo '<div class="form-field _wpro_dates_field_variable">
					<p class="form-row form-row-first">
						...
						<input type="text" class="_wpro_date_variable" id="_wpro_date_variable[' . esc_attr( $loop ) . ']" name="_wpro_date_variable[' . esc_attr( $loop ) . ']" value="' . esc_attr( $pre_order_date ) . '" placeholder="' . esc_html_x( 'DATE&hellip;', 'placeholder', 'product-pre-orders-for-woo' ) . ' YYYY-MM-DD" maxlength="10" pattern="' . esc_attr( apply_filters( 'woocommerce_date_input_html_pattern', '[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])' ) ) . '" />
						<input type="time" id="_wpro_time_variable[' . esc_attr( $loop ) . ']" name="_wpro_time_variable[' . esc_attr( $loop ) . ']" class="_wpro_time_variable" value="' . esc_attr( $sale_time ) . '">
					</p>';
        
        echo '<p class="form-field form-row form-row-last">
                        <label for="_wpro_label_variable' . esc_attr( $loop ) . '">' . esc_html__( 'Pre-Order label', 'product-pre-orders-for-woo' ) . '</label>' . wp_kses_post( wc_help_tip( esc_html__( 'Set a custom label to announce the pre-order product status. For example: Pre-Order Now!. Leaving it blankl, it will show the default label (Pre-Order)', 'product-pre-orders-for-woo' ) ) ) . '<input type="text"  value="' . esc_attr( $label ) . '" name="_wpro_label_variable[' . esc_attr( $loop )
             . ']" id="_wpro_label_variable' . esc_attr( $loop ) . '">
                    </p>
				   </div>';
        ?>
        <p class="form-field form-row form-row-first wpro-date-label-variable">
            ...
            <?php echo wp_kses_post( wc_help_tip( esc_html__( 'Use {availability_date} and {availability_time} to show when to remove the Pre-Order status.', 'product-pre-orders-for-woo' ) ) ); ?>
            <span class="wrap">
        ...
        <p class="form-field form-row form-row-last wpro-no-date-label-variable">
            ...
            <?php echo wp_kses_post( wc_help_tip( esc_html__( 'Message to be shown when no date is set. Leave it empty to use the global option.', 'product-pre-orders-for-woo' ) ) ); ?>
            <span class="wrap">
 		        	<input class="_wpro_no_date_label_variable"
                           ...
                           value="<?php echo esc_attr( $no_date_label ) ?>"
        ...
        <div class="form-field form-row form-row-first wpro-pre-order-variation">
            ...
            <div class=" wpro-manage-price hidden">
                ...
                <?php
                echo '<p class="form-field _wpro_price_variable[' . esc_attr( $loop ) . ']"> ' . wp_kses_post( wc_help_tip( esc_html__( 'Set the quantity to apply (Fixed or percentage). Type numbers only.', 'product-pre-orders-for-woo' ) ) ) . '<label>' . esc_html__( 'Adjustment amount', 'product-pre-orders-for-woo' ) . '</label>
					<span class="wrap">
    ...
    public function pre_order_save_simple_data( $post_id ) {
        if ( ! isset( $_POST['_wpro_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['_wpro_nonce'] ), 'wpro_product_meta_box' ) ) {
            return;
        }
        $_price_manually = isset( $_POST['_wpro_price_type'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_price_type'] ) ) : '';
        $_amount_price   = isset( $_POST['_wpro_amount_price'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_amount_price'] ) ) : '';
        $check_box       = isset( $_POST['simple_preorder'] ) && sanitize_text_field( wp_unslash( $_POST['simple_preorder'] ) ) ? 'yes' : 'no';
        $manage_price    = isset( $_POST['_wpro_manage_price'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_manage_price'] ) ) : '';
        $save_price      = isset( $_POST['_wpro_price'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_price'] ) ) : '';
        $save_label      = isset( $_POST['_wpro_label'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_label'] ) ) : '';
        $date_label      = isset( $_POST['_wpro_date_label'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_date_label'] ) ) : '';
        $no_date_label   = isset( $_POST['_wpro_no_date_label'] ) ? sanitize_text_field( wp_unslash( $_POST['_wpro_no_date_label'] ) ) : '';
        $_field_date     = isset( $_POST['_wpro_date'] ) ? strtotime( sanitize_text_field( wp_unslash( $_POST['_wpro_date'] ) ) ) : '';
        $_field_time     = isset( $_POST['_wpro_time'] ) ? self::pre_order_time( sanitize_text_field( wp_unslash( $_POST['_wpro_time'] ) ) ) : '';
